...
//    .LOAD(1'b0),
  ...
  localparam DETECT_EDGE = 8'b00000010;
  localparam CHECK_EDGE  = 8'b00000100;
  localparam INC_DELAY   = 8'b00001000;
  ...
  reg [7:0] error_count;
  reg [4:0] edge_idelay;     // idelay value at first rising edge
  reg [4:0] max_cnt;         // maximum error count observed
  reg prev_data;             // Previous value of data_out
  // Debug: store the trial_counter value for each idelay value.
  reg [511:0] trial_counter_history;
  reg [4:0] edge_pos;
  reg time_over;
...
//                cycle_counter <= 0;
                ...
                time_over <= 1'b0;
                state         <= CHECK_EDGE;
              end else if(cycle_counter[6] && time_over == 1'b0) begin
                time_over <= 1'b1;
              end else if(cycle_counter[6] && time_over == 1'b1) begin
                state <= INC_DELAY;
        ...
        CHECK_EDGE: begin
          if (cycle_counter[4:0] == edge_pos) begin
